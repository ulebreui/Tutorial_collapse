   ...
   match kernel: 
      ...
      case 0|1|2:
         ...
         if (kpol == 0):
            ...
            # numba version
            with numba_progress.ProgressBar(total=nbins, desc="Precomputing coagtabflux k=%d with numba"%(kpol)) as progress:
               compute_coagtabflux_numba(kernel,K0,Q,vecnodes,vecweights,nbins,kpol,massgrid,mat_coeffs_leg,tensor_tabflux_coag,progress)
            ...
            with numba_progress.ProgressBar(total=nbins, desc="Precomputing coagtabintflux k=%d with numba"%(kpol)) as progress:
               compute_coagtabintflux_numba(kernel,K0,Q,vecnodes,vecweights,nbins,kpol,massgrid,mat_coeffs_leg,tensor_tabintflux_coag,progress)
      ...
      case 0|1|2:
         ...
         if (kpol == 0):
            for i in range(ndthydro):
               gij,nsub,ndt = compute_coag_k0(eps,coeff_CFL,nbins,massgrid,gij,tensor_tabflux_coag,dthydro)
               tot_nsub += nsub
               tot_ndt += ndt
               time_coag += dthydro
               bar.update(i+1)
            ...
            for i in range(ndthydro):
               gij,nsub,ndt = compute_coag(eps,coeff_CFL,nbins,kpol,massgrid,massbins,gij,tensor_tabflux_coag,tensor_tabintflux_coag,dthydro)
...
def iterate_coag_kdv(kernel,K0,nbins,kpol,dthydro,ndthydro,coeff_CFL,Q,eps,massgrid,massbins,dv):
   """
   Function to iterate coagulation solver to reach the time ndthydro x dthydro 
   ...
   if (kernel == 3):
      ...
      if (kpol == 0):
         ...
         for i in range(ndthydro):
            gij,nsub,ndt = compute_coag_kdv(eps,coeff_CFL,nbins,kpol,massgrid,massbins,gij,tensor_tabflux_coag,tensor_tabintflux_coag,dv,dthydro)
